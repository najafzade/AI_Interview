...
import { Loader2, Send, CheckCircle2, User } from 'lucide-react';
...
export default function InterviewPage() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const output = await submitResponseAction(sessionId!, userMsg);
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-background">
        <Card className="w-full max-w-md p-8 shadow-xl border-2">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-headline font-bold mb-2">Technical Interview</h2>
            <p className="text-muted-foreground">Please enter your name to begin the assessment.</p>
          </div>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {isComplete && (
            <div className="flex flex-col items-center justify-center p-8 space-y-4 animate-in zoom-in duration-500">
              ...
              <p className="text-muted-foreground text-center">Thank you for your time. Your responses have been recorded for review.</p>
              <Link href="/">
      ...
      <div className="p-4 border-t bg-white">
        <div className="max-w-3xl mx-auto">
          <form onSubmit={handleSend} className="relative flex items-center space-x-2">
            <Input 
              placeholder={isComplete ? "Interview closed" : "Type your response..."}
              value={input}
          ...
          <p className="text-center text-[10px] text-muted-foreground mt-3 uppercase tracking-wider font-semibold">
            Interview Loop AI Assistant • Powered by Genkit
          </p>
        </div>
      </div>
    </div>
